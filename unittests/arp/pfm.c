...
pfm_retval_t pfm_init(int argc, char *argv[])
{
	...
	if (ret == -1)	{
		pfm_log_rte_err(PFM_LOG_EMERG,
				"Failed initializing lpm and route module"
				"Terminating");
		return PFM_FAILED;
	}
	pfm_trace_msg("initialised lpm and route module");
	*/
	ret = arp_init();
	if (ret == -1)	{
		pfm_log_msg(PFM_LOG_EMERG,
			    "Failed intializing arp module"
			    "Terminating");
		return PFM_FAILED;
	}
	pfm_trace_msg("initialised arp module");
