...
static arp_table_t arp_table_g;
static int arp_in = 0;
...
arp_init(void)	{
	...
	arp_table_g.hash_mapper	=	rte_hash_create(&hash_params);
	if (arp_table_g.hash_mapper == NULL)	{
	...
	pfm_trace_msg("arp_table_g initialized");
	return 0;
}

int arp_print_table(FILE *fp)	{
	fprintf(fp,"|  %-2s | %-12s | %-18s | %-8s | %-8s | \n",
		"ID","IP ADDRESS","MAC ADDRESS","LINK ID","#RETRIES");
	for (int i= 0;i < PFM_ARP_TABLE_ENTRIES;i++)	{
		fprintf(fp,"|  %-2d | %3d.%3d.%3d.%3d | %02X:%02X:%02X:%02X:%02X:%02X | %-8d | %-8d | \n",
				i,
				arp_table_g.entries[i].dst_ip_addr.addr_bytes[0],
				arp_table_g.entries[i].dst_ip_addr.addr_bytes[1],
				arp_table_g.entries[i].dst_ip_addr.addr_bytes[2],
				arp_table_g.entries[i].dst_ip_addr.addr_bytes[3],
				arp_table_g.entries[i].mac_addr.addr_bytes[0],
				arp_table_g.entries[i].mac_addr.addr_bytes[1],
				arp_table_g.entries[i].mac_addr.addr_bytes[2],
				arp_table_g.entries[i].mac_addr.addr_bytes[3],
				arp_table_g.entries[i].mac_addr.addr_bytes[4],
				arp_table_g.entries[i].mac_addr.addr_bytes[5],
				arp_table_g.entries[i].link_id,
				arp_table_g.entries[i].try_count);
...
arp_clear_entry(int key){
	...
	rte_ether_addr_copy(&empt_mac,&(arp_table_g.entries[key].mac_addr));
	ipv4_addr_copy(&empt_ip,&(arp_table_g.entries[key].dst_ip_addr));
	arp_table_g.entries[key].try_count = 0;
	arp_table_g.entries[key].link_id = 0;
	pfm_trace_msg("Cleared arp entries");
	ret = rte_timer_stop(arp_table_g.entries[key].refresh_timer);
	if (ret != 0)	{
		pfm_log_msg(PFM_LOG_ERR,
...
arp_request_send(int key)	{
	...
	// Pending 
	packet[0] = arp_table_g.entries[key].mac_addr.addr_bytes[0];
	packet[1] = arp_table_g.entries[key].mac_addr.addr_bytes[1];
	packet[2] = arp_table_g.entries[key].mac_addr.addr_bytes[2];
	packet[3] = arp_table_g.entries[key].mac_addr.addr_bytes[3];
	packet[4] = arp_table_g.entries[key].mac_addr.addr_bytes[4];
	packet[5] = arp_table_g.entries[key].mac_addr.addr_bytes[5];
...
refresh_callback(__rte_unused struct rte_timer * timer,void * called_args)	{
	...
	pfm_trace_msg("ARP entry timed out refresh call attempt :: %d",
		      arp_table_g.entries[args->key].try_count++);
	// If within retry limit retry and send an arp request
	if (arp_table_g.entries[args->key].try_count < 4)	{
		args->ticks = args->ticks*2;
		// Send an arp request
		// PENDING
		arp_request_send(key);
		rte_timer_reset(arp_table_g.entries[key].refresh_timer,
				ticks,
				PERIODICAL,
				rte_lcore_id(),
				refresh_callback,
...
int 
arp_process_reply(struct rte_mbuf *pkt,int link_id)	{
	...
	key = rte_hash_lookup_data(arp_table_g.hash_mapper,
				   (void *) &ip_lp,
				   NULL);
	...
	// 	1.b	If same value for src_ip just update the entry with new values
		if (ipv4_addr_equal(&src_ip_addr,&(arp_table_g.entries[key].dst_ip_addr)))	{
			rte_ether_addr_copy(&src_mac_addr,&(arp_table_g.entries[key].mac_addr));
			//ACHA
			arp_table_g.entries[key].link_id = link_id;
			arp_table_g.entries[key].try_count = 0;
			// Reset the timer
	...
	ret = rte_hash_count(arp_table_g.hash_mapper);
	...
	key = rte_hash_add_key(arp_table_g.hash_mapper,
			       (void *) &ip_lp);
	if (key <0)	{
		if (key == -EINVAL)	{
	...
	rte_ether_addr_copy(&src_mac_addr,&(arp_table_g.entries[key].mac_addr));
	ipv4_addr_copy(&src_ip_addr,&(arp_table_g.entries[key].dst_ip_addr));
	//Acha
	arp_table_g.entries[key].link_id = link_id;
	arp_table_g.entries[key].try_count = 0;
	...
	rte_timer_init(arp_table_g.entries[key].refresh_timer);
	rte_timer_reset(arp_table_g.entries[key].refresh_timer,
			ticks,
			PERIODICAL,
			rte_lcore_id(),
