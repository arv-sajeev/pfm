	   ...
	   pdus_setup_fail_rsp_info_t *fail_rsp)
{
	...
	if (ue_ctx->pdus_count == MAX_PDUS_PER_UE)
	{
		pfm_log_msg(PFM_LOG_ERR,"Overflow MAX_PDUS_PER_UE");
	...
	{
		if (ue_ctx->pdus_tunnel_list[i]->pdus_info.pdus_id == req->pdus_id)
		{	
			pfm_log_msg(PFM_LOG_ERR,"Attempt to add existing pdus");
			pdus_setup_fail_rsp_create(req,fail_rsp,
						FAIL_CAUSE_RNL_MULTIPLE_PDUS_ID);
			return PFM_FAILED;
		}
	}
